...
def main():
    ...
    if args.retrieval == 'nn':  # Standard nearest neighbor
        for i in range(0, len(src), BATCH_SIZE):
            ...
            # b = np.take_along_axis(similarities, index_array, axis=-1)
            b = np.sort(index_array[:,-5:])
            for k in range(j-i):
                translation1[src[i+k]] = b[k,:]
                translation[src[i+k]] = nn[k]
                # print(translation1[src[i+k]])
                # print(translation[src[i+k]])
    ...
    for i in src:
      a = list(trg_word2ind.keys())[list(trg_word2ind.values()).index(src2trg[i].pop())]
      b = []
      for j in range(5):
        b.append(list(trg_word2ind.keys())[list(trg_word2ind.values()).index(translation1[i][j])])
      print((a,b))
